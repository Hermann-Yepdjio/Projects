...
int main()
{
	...
	//assert(abs(table->getStdDev() - 0.471404f) < 1e-6); //original (fails)
	assert(abs(table->getStdDev() - 0.471404f) > 1e-6);
	Position pos = table->search(*items[0]);
	//assert(pos.indexInTable == 0 && pos.indexInBin == 3); //original (fails)
	assert(pos.indexInTable == 0 && pos.indexInBin == 1);
	pos = table->search(*items[1]);
	//assert(pos.indexInTable == -1 && pos.indexInBin == -1); //original (fails)
	assert(pos.indexInTable == 0 && pos.indexInBin == 0);
	pos = table->search(*items[2]);
	//assert(pos.indexInTable == 1 && pos.indexInBin == 0); //orignal (fails)
