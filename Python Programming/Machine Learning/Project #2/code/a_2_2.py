...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(6, 16, 3, stride=1, padding=0)
...
#        x = F.tanh(self.fc2(x))
        ...
        x = x.view(-1, 16 * 6 * 6)  #flatten the image input (output of the 2nd pooling later)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
#                display_feature_maps(model, image, file_name)
            ...
            file_name = "./Images/Epoch_VS_Loss/ReLU_" + loss_function_label[criterions.index(criterion)] + "_" + str(l_rate) + ".png"
