...
#        x = F.sigmoid(self.fc2(x))
        ...
        x = self.pool1(F.tanh(self.conv1(x))) #Apply pooling after conv
        x = self.pool2(F.tanh(self.conv2(x))) #apply pooling after conv
        x = x.view(-1, 16 * 5 * 5)  #flatten the image input (output of the 2nd pooling later)
        x = F.tanh(self.fc1(x))
        x = F.tanh(self.fc2(x))
#
...
#            file_name = "./Images/Epoch_VS_Loss/Tanh_" + loss_function_label[criterions.index(criterion)] + "_" + str(l_rate) + ".png"
