...
class CircularCoord(object):
    ...
    def read_data(self):
        ...
        #print(data)
        pd.DataFrame(data).to_csv('normalized_' + self.filename + '.csv',
                    index = False)
    ...
    def drawCoord(self):
        ...
        self.cvs.drawCircle(self.center, r, self.startAngle)
        angle = np.deg2rad(self.startAngle)
        angleInc = np.deg2rad(float(360) / float(self.dim))
        for i in range(self.dim):
            self.cvs.moveTo(self.center.x + 0.95 * r * np.cos(angle),
                            self.center.y + 0.95 * r * np.sin(angle))
            self.cvs.lineTo(self.center.x + 1.05 * r * np.cos(angle),
                            self.center.y + 1.05 * r * np.sin(angle))
            self.cvs.drawString2(self.center.x + (1 + 1/6) * r * np.cos(angle - 0.5 * angleInc),
                                 self.center.y + (1 + 1/6) * r * np.sin(angle - 0.5 * angleInc),
                                 'X' + str(i + 1))
            angle -= angleInc
        ...
        for c in self.data.keys():
            ...
            for d in self.data[c]:
                ...
                for n, i in enumerate(d):
                    ...
                    if n == 0:
                        self.cvs.moveTo(self.center.x + r * np.cos(dataAngle),
                                        self.center.y + r * np.sin(dataAngle))
                    else:
                        self.cvs.lineTo(self.center.x + r * np.cos(dataAngle),
                                        self.center.y + r * np.sin(dataAngle))
    ...
    def drawLabels(self):
        self.cvs.setViewport(int(screen_width - (screen_width - screen_height) / 2 - 30),
                             screen_width, 0, screen_height)
        self.cvs.setWindow(0, screen_width, 0, screen_height)
        for n, k in enumerate(self.classColor.keys()):
            ...
            gl.glVertex2d(55, (screen_height - 45) - (25 * (n + 1)))
            gl.glEnd()
            gl.glFlush()
...
def display():
    cvs.setWindow(-1.5 * r, 1.5 * r, -1.5 * r, 1.5 * r)
