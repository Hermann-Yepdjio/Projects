...
#for i in range(generations):
 #   parents, mc = advance_generation(parents, 'one child', male_portion, fertility, law_breakers, wanted_children)
...
for i in range(generations):
    ...
    print '%3d: %d (max children in a family: %d)' % \
          (i+1, len(parents), mc)
    if (i>0):
        print "r = ", (len(parents)/float(Xn)) -1
        r += ((len(parents)/float(Xn))-1)/9
    Xn = len(parents)
print "We can conclude that r = ", r, "approximately"
