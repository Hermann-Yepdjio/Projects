...
(flatten (l) (if (@ isnil l)
                 (@ nil)
                 (@ append (@ head l) (@ flatten (@ tail l)))))
                   
(unzip (l) (if (@ isnil l)
               (pair (@ nil) (@ nil))
               (let hd (@ head l)
                    (let tls (@ unzip (@ tail l))
                         (pair (@ cons (fst hd) (fst tls))
                               (@ cons (snd hd) (snd tls)))))))

(nreverseHelper (l p) (if (@ isnil l) p (@ nreverseHelper (@ tail l) (@ settail l p))))
(nreverse (l) (@ nreverseHelper l (@ nil)))
