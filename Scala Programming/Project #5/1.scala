/*
The solution template has an interpreter for a language very similar to E4 from Lab 4, but with one modification:
the actual arguments to functions must be variables (either global or let-bound), not arbitrary expressions.
(To pass an arbitrary expression value, the programmer can first bind it to a variable using let.)

Like all the other languages we have seen, this one uses call-by-value for all function parameters. Your task is to change it to use call-by-reference everwhere instead. We’ll call the resulting language E6. To fix the template interpreter to do this:

Change the interpE code for Apply to pass the location of each parameter rather than its value. In order to make this possible while still using the stack to communicate between caller and callee, add a new case class LocationV extending Value that carries a Location.

Change the interpFun code to bind the formal parameters directly to the passed locations, rather than to the parameter stack slots.
(In the resulting interpreter, only let-bound variables will have a storage slot in the stack!)

These are the only pieces of the interpreter that need to change. The whole change is just a few lines of code.

Consider the swap function (discussed in lecture 4a, defined in the snippet below, and also used in a complete program in the template tests):

(swap (x y) (let z x (block (:= x y) (:= y z)))
Make sure to test that functions like swap now have (the desired) effect in the caller’s context.

Hint: The trickiest part of this problem occurs because StackFrameOffset locations are relative to the current frame pointer,
which changes each time a function is entered: the caller will need to be prepared to adjust these locations. Note that it is
perfectly OK for offsets to be negative!*/
...
object Parser {
  ...
  def parseE(sexpr: SExpr) : Expr = sexpr match {
    ...
    //case SList(SSym("loc") :: SSym(a) :: Nil) => LocationV(a)
...
object Printer {
  ...
  def unparse(expr: Expr) : SExpr = expr match {
    ...
    //case LocationV(a) => SList(SSym("loc") :: SSym(a) :: Nil)
  }
...
object Interp {
  ...
  case class LocationV(a:Location) extends Value
  ...
  def interp(p:Program,debug:Int = 0): Int = {
    ...
    def extractLocation(v:Value) = v match
    {
        case LocationV(a) => a
        case _ => throw InterpException("non-LocationV argument to ectractLocation")
    }
    ...
    def interpFun(fname:String,fp:Int, old_fp:Int, argCount:Int) : Value = {
      if (debug > 1) 
      ...
      for (x <- fd.params) 
      {
        val loc = extractLocation(getLocation(fp, StackFrameOffset(offset)))
        benv = benv + (x -> loc) //StackFrameOffset(offset))
        offset += 1
      }
      // evaluate function body 
      val v = interpE(benv,old_fp,fd.body)
      // return result
      if (debug > 1)
        println("returning " + v)
      v
    ...
    def interpE(env:Env,fp:Int,e:Expr) : Value = {
      ...
      val r = e match {
        //case LocationV(a) => interpVar(env, a)
        case Num(n) => NumV(n)
        ...
        case Apply(f,xs) => {
          // evaluate  arguments and push them on the stack, creating a new frame
          val fpNew = stack.push(xs.length)
          var offset = 0
          for (x <- xs) {
            ///val v = getLocation(fp,interpVar(env,x))
            val loc = LocationV(interpVar(env, x))
            setLocation(fpNew, StackFrameOffset(offset), loc)
            offset += 1
          }
          val v = interpFun(f,fpNew, fp, xs.length)
          // restore stack
          stack.pop(xs.length)
          v
        }
