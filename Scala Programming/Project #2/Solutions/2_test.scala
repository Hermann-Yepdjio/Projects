 ...
 */
  ...
  // keep this test to check for random breakage
  ...
  test("parse and,or,not, xor, imp") {
    assertResult(Imp(And(Bool(true),Bool(false)),Not(Or(Bool(false),Xor(Bool(true),Bool(true)))))) {parse("(-> (& T F) (~ (| F (^ T T))))")}
  }

  test("parse or with subexpressions") {
    assertResult(Or(And(Bool(true),Bool(false)),Xor(Bool(false),Bool(true)))) {parse("(| (& T F) (^ F T))")}
  }

  test("parse imp with subexpressions") {
    assertResult(Imp(And(Bool(true),Bool(false)),Xor(Bool(false),Bool(true)))) {parse("(-> (& T F) (^ F T))")}
  }
 ...
 */
  ...
  test("print and,or,not,xor,imp") {
    assertResult ("(-> (& T F) (~ (| F (^ T T))))")  {Imp(And(Bool(true),Bool(false)),Not(Or(Bool(false),Xor(Bool(true),Bool(true))))).toString}
  }
  
  test("print or with subexpressions") {
    assertResult ("(| (& T F) (^ F T))") {Or(And(Bool(true),Bool(false)),Xor(Bool(false),Bool(true))).toString} 
  }

  test("print imp with subexpressions") {
    assertResult ("(-> (& T F) (^ F T))")  {Imp(And(Bool(true),Bool(false)),Xor(Bool(false),Bool(true))).toString} 
  }
 ...
 */
  ...
  test ("correctly interpret imp with subexpression") {
    assertResult(false) {Process.process("(-> (^ T F) (& F T))")}
  }
  
  test("correctly interpret or with subexpressions") {
    assertResult(true) {Process.process("(| (& T F) (^ F T))")}
  }
  ...
  test("correctly interpret another complex expression") {
    assertResult(true) {Process.process("(| (-> (~ (| T F)) F) (~ F))")}
  }
}
