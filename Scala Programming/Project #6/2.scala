/*
The solution template contains the same interpreter as in part (a), which does not support truly first-class functions, because closures can refer to stack-allocated values.

(b) Your task for this section is to solve this problem as follows:

In buildClosure, copy each stack-allocated value in the environment into a new heap location, and create a new environment whose entries point to these heap locations instead of the stack. (The HeapAddr location type will be useful here.) Incorporate this new environment into the closure.

This will allow you to significantly simplify interpFun, since closure environments should no longer ever contain StackFrameOffset locations. As one aspect of making this change, you should remove the fp field from closures, as it will no longer be needed.

Adjust the interpretation of LetRec expressions to place the closure value for the recursively-bound function into the heap directly, rather than stack-allocating it.

No other changes are required.

Among other tests, you should confirm that your test program from part (a) now works properly.

Notes: (1) This copying approach works because our language has no assignment operation, so the values of parameters and let-bound variables never change from their initial values. Hence, there is no danger that a variable’s value might change after we copy it.
(2) We could avoid the need to copy variables by simply allocating all values in the heap in the first place. But the scheme we’re using has the advantage of keeping variables on the stack in the common case where no nested function closure needs to be built.
(3) We could do less copying if our closure environments contained entries only for the variables actually mentioned in the nested function body (its free variables). This would just require a (straightforward) scan of the function body to identify these free variables.*/
...
object Interp {
  ...
  case class ClosureV(xs:List[String],b:Expr,env:Env) extends Value
  ...
  def interp(p:Program,debug:Int = 0): Int = {
    ...
    def buildClosure(fp:Int,params:List[String],body:Expr,env:Env) = 
    {
      ...
      for ((x,l) <- env) l match 
      {
        ...
        ClosureV(params,body,benv)
    }

    def interpFun(params:List[String],body:Expr,cenv:Env,fp:Int,argCount:Int) : Value = {
      if (debug > 1)
      ...
      val r = e match {
        ...
        case Apply(e,es) => interpE(env,fp,e) match {
          case ClosureV(xs,b,cenv) => {
            ...
            val v = interpFun(xs,b,cenv,fpNew,es.length)
            // restore stack
            stack.pop(es.length)
            v
          }
        ...
        case LetRec(f,Fun(xs,b),e) => 
        {
            ...
            val a = heap.allocate(1)
            val renv = env + (f -> HeapAddr(a))

            heap.set(a, buildClosure(fp,xs,b,renv))
