class Student implements Comparable<Student>
{
   ...
   *
   * @return -1, 1  if different names, or significant difference in grade
   *          0, if name is the same, no significant difference in gpa.
   */
  public int compareTo (Student otherStudent)
  {
    final double DELTA = 0.001; // the  tolerance
    
// if this.name alphabetically first returns -1 (String comparison)
    if (name.compareTo (otherStudent.name) < 0)
      return -1;
    if (name.compareTo (otherStudent.name) > 0)
      return 1;

//  If there is a siginficant gpa difference 
    if (gpa - otherStudent.gpa > DELTA)
      return -1;
    if (otherStudent.gpa - gpa > DELTA)
      return 1;

// names the same and grades within DELTA
    return 0;
  } // method compareTo
  
  /**
   * Determines if this Student objects name and grade point average are
