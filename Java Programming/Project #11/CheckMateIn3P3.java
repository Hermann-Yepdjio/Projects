...
public class CheckMateIn3P3 
{
	...
	public static void display() throws UnsupportedEncodingException, IOException
	{
		
		Mate=new ArrayList<String>();
		Scanner scan=new Scanner(new File("C:\\Users\\Hermann\\Documents\\position1.fen"));
		if (scan.hasNext())
			board =scan.next();
		try (Writer writer = new BufferedWriter(new OutputStreamWriter( new FileOutputStream("Solutions.txt"), "utf-8"))) 
	      {
			...
			Backtracking(CM.getPositions(board), CM.getPossibleMoves(CM.getPositions(board)), 0, "", false);
			if(Mate.isEmpty())
			{
	...
	public static void Backtracking(ArrayList<Position> positions, ArrayList<Position> possibleMoves, int count, String solution, boolean condition)
	{
			...
			for (Position pos: possibleMoves) // execute possible moves
			{
				...
				else if( Character.isUpperCase(pos.getPiece())&&!condition) //white makes a move
				{
					newPositions.clear();
					...
					for(Position pos3:newPossibleMoves) //to operate a possible move
					{
						...
						else if(Character.isLowerCase(pos3.getPiece()) && count<2 ) //black moves
						{
							...
							if(CM.isBMated2(newPositions1) && !condition) //check if b is mated
							{
							...
							{
								Backtracking(newPositions1, newPossibleMoves1, count+1, solution+ " "+(count+1)+ ". " +pos.getPosition()+" + " +pos3.getPosition()+"  ", condition);
