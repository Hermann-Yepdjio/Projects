...
//compute one elt of the resulting matrix from matrix multiplication
...
//compute one row of the resulting matrix from matrix multiplication
void *compute_one_row(void *arg1)	
{
	...
	Arg *arg = (Arg *)arg1; 
	for(int j = 0; j < arg->matrix_2->n_cols; j++)
        {
		arg->j = j;
        	compute_one_elt(arg);
	}
