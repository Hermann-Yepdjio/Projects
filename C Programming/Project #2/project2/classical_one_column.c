...
void compute_one_elt(Arg *arg)
{
	long double sum_values = 0;
	for(int k = 0; k < arg->n_mult; k++)
		sum_values += arg->matrix_1->values[k][arg->i] * arg->matrix_2->values[arg->j][k];
	arg->result->values[arg->j][arg->i] = sum_values;
}

//compute one column of the resulting matrix from matrix multiplication
void *compute_one_column(void *arg1)
{
	while(!is_ready_1)
	{
		sleep(0.001);
	...
	Arg *arg = (Arg *)arg1;
        for(int j = 0; j < arg->matrix_2->n_rows; j++)
        {
                arg->j = j;
                compute_one_elt(arg);
        }
...
{
	pthread_t thread_IDs[result->n_rows];
	int count = 0;
	...
	for (int i = 0; i < matrix_1->n_cols; i++)
	{
		...
		if(!arg)
			perror("Memory Allocation Error!");
	      	...
	      	arg->result = result, arg->matrix_1 = matrix_1, arg->matrix_2 = matrix_2, arg->i = i, arg->j = 0, arg->n_mult = n_multiplications;
		
		int thread_return = pthread_create(&thread_IDs[count], NULL, compute_one_column, (void *)arg);
